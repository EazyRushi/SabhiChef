...
import { ArrowLeft, MapPin, CreditCard, Package, Tag, X, CheckCircle } from 'lucide-react'
import Navbar from '@/components/navbar'
import { validateCoupon, placeOrder } from '@/lib/supabase/actions'
...
export default function CheckoutClient({ savedAddresses }: Props) {
  ...
  const [selectedAddressId, setSelectedAddressId] = useState(savedAddresses.find(a => a.is_default)?.id || '')
  ...
  const handleApplyCoupon = async () => {
    ...
    if ('error' in result) { setCouponError(result.error ?? 'Invalid coupon'); setDiscount(0); setCouponId('') }
      ...
      setCouponSuccess(`✓ Saved ₹${result.discount}!`)
    }
  }

  const getActiveAddress = () => {
    if (useSaved && selectedAddressId) {
      const a = savedAddresses.find(addr => addr.id === selectedAddressId)
      if (a) return { name: a.full_name, phone: a.phone, line1: a.address_line1, line2: a.address_line2 || '', city: a.city, state: a.state, pincode: a.pincode }
    }
    return address
  }
  ...
  const handlePlaceOrder = async () => {
    ...
    const result = await placeOrder(
      items.map(i => ({ productId: i.product.id, productName: i.product.name, productPrice: i.product.price, quantity: i.quantity })),
      activeAddress,
      payment,
      subtotal,
      shipping,
    ...
    if ('error' in result) { setPlacing(false); alert(result.error); return }
    clearCart()
    router.push(`/order-success?id=${result.orderId}&order=${result.orderNumber}`)
  }
    ...
    <><Navbar />
      <div className="min-h-screen pt-20" style={{ background: '#FFF9ED' }}>
        <div className="max-w-4xl mx-auto px-6 py-12">
          <Link href="/cart" className="inline-flex items-center gap-2 text-sm font-bold text-[#995424] hover:text-[#F47B40] mb-6"><ArrowLeft className="w-4 h-4" /> Back to Cart</Link>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              ...
              {step === 1 && (
                <div className="bg-white border-2 border-[#1e0f00] rounded-2xl p-6 shadow-[4px_4px_0_#1e0f00]" style={{ borderWidth: 2.5 }}>
                  ...
                  {savedAddresses.length > 0 && (
                    <div className="mb-5">
                      <p className="text-xs font-bold text-[#995424] uppercase tracking-wider mb-3">Saved Addresses</p>
                      <div className="space-y-2">
                        {savedAddresses.map(addr => (
                          <label key={addr.id} className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition ${selectedAddressId === addr.id && useSaved ? 'border-[#F47B40] bg-[#F47B40]/5' : 'border-[#995424]/10 hover:border-[#995424]/20'}`}>
                            <input type="radio" name="savedAddr" checked={selectedAddressId === addr.id && useSaved} onChange={() => { setSelectedAddressId(addr.id); setUseSaved(true) }} className="accent-[#F47B40] mt-1" />
                            <div>
                              <p className="text-sm font-black text-[#1e0f00]">{addr.label} — {addr.full_name}</p>
                              <p className="text-xs text-[#7a4a20]">{addr.address_line1}, {addr.city} - {addr.pincode}</p>
                            </div>
                          </label>
                        ))}
                        <label className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition ${!useSaved ? 'border-[#F47B40] bg-[#F47B40]/5' : 'border-[#995424]/10 hover:border-[#995424]/20'}`}>
                          <input type="radio" name="savedAddr" checked={!useSaved} onChange={() => setUseSaved(false)} className="accent-[#F47B40]" />
                          <span className="text-sm font-black text-[#1e0f00]">Use a new address</span>
                        </label>
                      </div>
                    </div>
                  )}
                  ...
                  <button onClick={() => setStep(2)} className="mt-6 bg-[#F47B40] text-white font-black py-3 px-8 rounded-full border-2 border-[#1e0f00] shadow-[3px_3px_0_#1e0f00] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all" style={{ borderWidth: 2.5 }}>Continue to Payment →</button>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="bg-white border-2 border-[#1e0f00] rounded-2xl p-6 shadow-[4px_4px_0_#1e0f00]" style={{ borderWidth: 2.5 }}>
                  <h2 className="font-black text-[#1e0f00] mb-5">💳 Payment & Coupon</h2>
                  ...
                  {/* Coupon */}
                  <div className="bg-[#FFF9ED] rounded-xl p-4 border border-[#995424]/10">
                    <div className="flex items-center gap-2 mb-2"><Tag className="w-4 h-4 text-[#F47B40]" /><span className="text-sm font-black text-[#1e0f00]">Have a coupon?</span></div>
                    <div className="flex gap-2">
                      <input value={couponCode} onChange={e => { setCouponCode(e.target.value.toUpperCase()); setCouponError(''); setCouponSuccess('') }}
                        placeholder="Enter code (e.g. WELCOME10)"
                        className="flex-1 px-4 py-2 rounded-xl border-2 border-[#995424]/20 bg-white text-sm font-semibold focus:outline-none focus:border-[#F47B40] uppercase" />
                      <button onClick={handleApplyCoupon} className="px-4 py-2 bg-[#16703A] text-white font-black text-sm rounded-xl border-2 border-[#1e0f00]">Apply</button>
                      {discount > 0 && <button onClick={() => { setDiscount(0); setCouponCode(''); setCouponId(''); setCouponSuccess('') }} className="p-2 rounded-xl border-2 border-[#DD2D2B]/20 hover:bg-red-50"><X className="w-4 h-4 text-[#DD2D2B]" /></button>}
                    </div>
                    {couponError && <p className="text-xs text-[#DD2D2B] font-semibold mt-2">{couponError}</p>}
                    {couponSuccess && <p className="text-xs text-[#16703A] font-bold mt-2 flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5" />{couponSuccess}</p>}
                  </div>
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button onClick={() => setStep(3)} className="bg-[#F47B40] text-white font-black py-3 px-8 rounded-full border-2 border-[#1e0f00] shadow-[3px_3px_0_#1e0f00] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all" style={{ borderWidth: 2.5 }}>Review Order →</button>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="bg-white border-2 border-[#1e0f00] rounded-2xl p-6 shadow-[4px_4px_0_#1e0f00]" style={{ borderWidth: 2.5 }}>
                  ...
                  <div className="bg-[#FFF9ED] rounded-xl p-4 mb-4">
                    ...
                    <p className="text-sm font-bold text-[#1e0f00]">{activeAddress.name}</p>
                    <p className="text-xs text-[#7a4a20]">{activeAddress.line1} {activeAddress.line2}</p>
                    <p className="text-xs text-[#7a4a20]">{activeAddress.city}, {activeAddress.state} – {activeAddress.pincode}</p>
                  </div>
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button onClick={handlePlaceOrder} disabled={placing} className="bg-[#16703A] text-white font-black py-3 px-8 rounded-full border-2 border-[#1e0f00] shadow-[3px_3px_0_#1e0f00] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all disabled:opacity-60" style={{ borderWidth: 2.5 }}>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white border-2 border-[#995424]/20 rounded-2xl p-5 sticky top-24">
                ...
                <div className="border-t border-[#995424]/10 mt-4 pt-4 space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-[#7a4a20]">Subtotal</span><span className="font-bold">₹{subtotal}</span></div>
                  {discount > 0 && <div className="flex justify-between"><span className="text-[#16703A]">Discount</span><span className="font-bold text-[#16703A]">–₹{discount}</span></div>}
                  <div className="flex justify-between"><span className="text-[#7a4a20]">Shipping</span><span className="font-bold">{shipping === 0 ? 'Free' : `₹${shipping}`}</span></div>
                </div>
                <div className="border-t border-[#995424]/10 mt-4 pt-4 flex justify-between">
                  <span className="font-black text-[#1e0f00]">Total</span>
                  <span className="font-black text-xl text-[#DD2D2B]">₹{total}</span>
                </div>
              </div>
            </div>
          </div>
