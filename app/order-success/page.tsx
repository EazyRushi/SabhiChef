...
function OrderSuccessContent() {
  ...
  return (
    <div className="max-w-lg mx-auto text-center px-6 py-24">
      ...
      <p className="text-[#5a3010] font-semibold mb-2">Thank you! We&apos;re preparing your homemade goodness with love.</p>
      ...
      <div className="bg-white border-3 border-[#1e0f00] rounded-2xl p-6 mt-8 shadow-[5px_5px_0_#1e0f00]" style={{ borderWidth: 3 }}>
        ...
        <p className="text-2xl font-black text-[#F47B40]">{orderNumber}</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-8 justify-center">
        {orderNumber !== 'Your Order' && (
          <Link href={`/track/${orderNumber}`} className="flex items-center justify-center gap-2 bg-[#F47B40] text-white font-black py-3 px-6 rounded-full border-2 border-[#1e0f00] shadow-[3px_3px_0_#1e0f00] hover:translate-y-[-2px] transition-all" style={{ borderWidth: 2.5 }}>
            <Package className="w-4 h-4" /> Track Order
          </Link>
        )}
        <Link href="/account/orders" className="flex items-center justify-center gap-2 bg-[#16703A] text-white font-black py-3 px-6 rounded-full border-2 border-[#1e0f00] shadow-[3px_3px_0_#1e0f00] hover:translate-y-[-2px] transition-all" style={{ borderWidth: 2.5 }}>
