...
export default function ProductDetail() {
  ...
  if (!product) {
    return (
      <><Navbar />
        <div className="min-h-screen pt-24 text-center" style={{background:'#FFF9ED'}}>
          <p className="text-2xl font-black text-[#995424]">Product not found</p>
          <Link href="/shop" className="text-[#F47B40] font-bold mt-4 inline-block">← Back to Shop</Link>
        </div>
      <Footer /></>
    )
  }
      ...
      <div className="min-h-screen pt-20" style={{ background: '#FFF9ED' }}>
        <div className="max-w-6xl mx-auto px-6 py-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Image */}
            <div className="bg-white border-3 border-[#1e0f00] rounded-3xl overflow-hidden shadow-[6px_6px_0_#1e0f00] aspect-square flex items-center justify-center text-8xl" style={{borderWidth:3, background:'linear-gradient(135deg, #FEE47220, #F47B4020)'}}>
            ...
            <div>
              ...
              <div className="flex gap-4 mt-6">
                <div className="flex items-center gap-2 bg-white border-2 border-[#995424]/20 rounded-xl px-4 py-2">
                  <Weight className="w-4 h-4 text-[#995424]" />
                  <span className="text-sm font-bold text-[#995424]">{product.weight}</span>
                </div>
                <div className="flex items-center gap-2 bg-white border-2 border-[#995424]/20 rounded-xl px-4 py-2">
                  <Users className="w-4 h-4 text-[#995424]" />
                  <span className="text-sm font-bold text-[#995424]">{product.servings}</span>
                </div>
                <div className="flex items-center gap-2 bg-white border-2 border-[#995424]/20 rounded-xl px-4 py-2">
                  <Clock className="w-4 h-4 text-[#995424]" />
                  <span className="text-sm font-bold text-[#995424]">{product.prep_time}</span>
                </div>
              </div>
              ...
              <button onClick={handleAdd}
                className={`mt-6 flex items-center gap-3 font-black text-lg px-8 py-4 rounded-full border-3 transition-all ${
                  added
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {reviews.map(r => (
                  <div key={r.id} className="bg-white border-2 border-[#995424]/10 rounded-2xl p-5">
                    ...
                    <p className="text-xs text-[#995424] font-bold mt-2">— {r.profile?.name}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {related.map(p => (
                  <Link key={p.id} href={`/product/${p.id}`} className="bg-white border-2 border-[#1e0f00] rounded-xl p-4 shadow-[3px_3px_0_#1e0f00] hover:translate-y-[-3px] hover:shadow-[3px_6px_0_#1e0f00] transition-all">
                    <div className="text-3xl text-center mb-2">{p.category === 'breakfast' ? '🌅' : p.category === 'meals' ? '🍲' : p.category === 'sweet' ? '🍰' : '🌿'}</div>
